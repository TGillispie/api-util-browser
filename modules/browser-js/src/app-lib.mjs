...
const RespUtil = function() {
   ...
   */
  function text(resp) {
    if (! resp.text && resp.data) {    // Axios support.
      return resp.data
    }
    return resp.text()    // fetch support
  }
   ...
   */
  function json (resp) {
    if (! resp.json && resp.data) {     // Axios support
      return JSON.parse(resp.data)
    }
    return resp.json()    // fetch support
  }

  async function dumpBodyText (resp) {
    const txt = await text(resp)
    console.log(txt)
    return resp
  }
